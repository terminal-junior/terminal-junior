# 👋 Olá — eu sou o Junior (terminal-junior)

Sou Analista de Infraestrutura e Suporte de TI e estudante de Ciência da Computação.  
Tenho mais de 12 anos de experiência com Linux, infraestrutura, redes e automação, sempre buscando construir ambientes estáveis, seguros e bem documentados.  
Curto transformar caos em organização — automatizando, monitorando e otimizando sistemas.

---

## 💡 O que me motiva

- Criar e manter ambientes confiáveis — seja Linux ou Windows  
- Automatizar tarefas repetitivas com Shell e Python  
- Monitorar, observar e entender ambientes reais  
- Trabalhar com infraestrutura moderna, alta disponibilidade e boas práticas  
- Compartilhar conhecimento por meio de documentação e projetos

---

## 🧰 Projetos recentes

- [**zabbix_grafana**](https://github.com/terminal-junior/zabbix_grafana) — Guia e scripts para deploy de Zabbix + Grafana com PostgreSQL.  
- [**nginx_web_server**](https://github.com/terminal-junior/nginx_web_server) — Templates e automações para servidores NGINX, incluindo proxy reverso, SSL/TLS e otimização.  
- [**hcaparecida**](https://github.com/terminal-junior/hcaparecida) — Automação em Python para atualização diária de relatórios/censos.  
<!--
- **Infra Lab** — Ambientes de laboratório com Proxmox, NixOS, Rocky Linux e automações diversas.  
-->
> 📌 Veja os repositórios — cada projeto possui um README explicando propósito, uso e minha contribuição.

---

## ⚙️ Tecnologias & Ferramentas

### 🖥️ **Sistemas Operacionais**
![FreeBSD](https://img.shields.io/badge/FreeBSD-AB2B28?logo=freebsd&logoColor=white)
![Linux](https://img.shields.io/badge/Linux-000?logo=linux&logoColor=white)
![Rocky Linux](https://img.shields.io/badge/Rocky_Linux-000?logo=linux&logoColor=white)
![Debian](https://img.shields.io/badge/Debian-D70A53?logo=debian&logoColor=white)
![Fedora](https://img.shields.io/badge/Fedora-294172?logo=fedora&logoColor=white)
![NixOS](https://img.shields.io/badge/NixOS-5273FF?logo=nixos&logoColor=white)
![Ubuntu Server](https://img.shields.io/badge/Ubuntu_Server-E95420?logo=ubuntu&logoColor=white)
![Windows 10](https://img.shields.io/badge/Windows_10-0078D6?logo=windows&logoColor=white)
![Windows Server](https://img.shields.io/badge/Windows_Server-0078D6?logo=windows&logoColor=white)

---

### 🧩 **Virtualização, HA e Containers**
![Proxmox](https://img.shields.io/badge/Proxmox-E34F26?logo=proxmox&logoColor=white)
![Ceph](https://img.shields.io/badge/Ceph-003263?logo=ceph&logoColor=white)
![QEMU](https://img.shields.io/badge/QEMU-FF6600?logo=qemu&logoColor=white)
![KVM](https://img.shields.io/badge/KVM-000000?logo=linux&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-2496ED?logo=docker&logoColor=white)
![Podman](https://img.shields.io/badge/Podman-892A2A?logo=podman&logoColor=white)
![Oracle VirtualBox](https://img.shields.io/badge/VirtualBox-183A61?logo=virtualbox&logoColor=white)
![Ventoy](https://img.shields.io/badge/Ventoy-2F7DCF?logoColor=white)

---

### 🌐 **Redes & Wireless**
![Aruba](https://img.shields.io/badge/Aruba_Networks-FF6A00?logo=arubanetworks&logoColor=white)
![HPE Aruba](https://img.shields.io/badge/HPE_Aruba-00A4EF?logo=hewlettpackardenterprise&logoColor=white)
![Ruckus](https://img.shields.io/badge/Ruckus-000000?logo=wireshark&logoColor=white)
![Ubiquiti](https://img.shields.io/badge/Ubiquiti-Unifi-0559C9?logo=ubiquiti&logoColor=white)
![TP-Link](https://img.shields.io/badge/TP--Link-4ACBD6?logo=tp-link&logoColor=white)
![MikroTik](https://img.shields.io/badge/MikroTik-293239?logo=mikrotik&logoColor=white)
![ZeroTier](https://img.shields.io/badge/ZeroTier-FF8800?logo=zerotier&logoColor=white)

---

### 🔧 **Infraestrutura, Serviços & Monitoramento**
![NGINX](https://img.shields.io/badge/NGINX-009639?logo=nginx&logoColor=white)
![Zabbix](https://img.shields.io/badge/Zabbix-CC0000?logo=zabbix&logoColor=white)
![Grafana](https://img.shields.io/badge/Grafana-0AC1D9?logo=grafana&logoColor=white)
![Pi-Hole](https://img.shields.io/badge/Pi--hole-96060C?logo=pihole&logoColor=white)
![Samba](https://img.shields.io/badge/Samba_4-354A5F?logo=samba&logoColor=white)
![RSAT](https://img.shields.io/badge/RSAT-000000?logo=windows&logoColor=white)

---

### 🛠️ **Ferramentas & Acesso Remoto**
![SSH](https://img.shields.io/badge/SSH-3E9C35?logo=openssh&logoColor=white)
![WinSCP](https://img.shields.io/badge/WinSCP-008000?logo=winscp&logoColor=white)
![FileZilla](https://img.shields.io/badge/FileZilla-BF0000?logo=filezilla&logoColor=white)
![AnyDesk](https://img.shields.io/badge/AnyDesk-EF443B?logo=anydesk&logoColor=white)
![TeamViewer](https://img.shields.io/badge/TeamViewer-0E8EE9?logo=teamviewer&logoColor=white)
![UltraVNC](https://img.shields.io/badge/UltraVNC-39499E?logoColor=white)
![Advanced IP Scanner](https://img.shields.io/badge/Advanced%20IP%20Scanner-0A7BDC?logoColor=white)

---

### 💻 **Edição & Scripting**
![VS Code](https://img.shields.io/badge/VS%20Code-007ACC?logo=visualstudiocode&logoColor=white)
![Nano](https://img.shields.io/badge/Nano-4A90E2?logo=gnu&logoColor=white)
![Shell Script](https://img.shields.io/badge/Shell-121011?logo=gnu-bash&logoColor=white)
![Python](https://img.shields.io/badge/Python-3776AB?logo=python&logoColor=white)

---

## 📨 Notificações & Automações
- Notificações do Zabbix via **Telegram** e **E-mail**  
- Pequenas automações em **Shell Script** e **Python**  
- Scripts para deploy, backup, manutenção e setup de ambiente

---

## 🎯 Objetivo Profissional

Atuar com administração de sistemas e infraestrutura, com foco em automação, alta disponibilidade, segurança e monitoramento.  
Busco evoluir continuamente, contribuir com projetos open source e construir soluções robustas e escaláveis.

---

## 📫 Contato & Informações

- Localidade: Recife / PE, Brasil  
- Disponível para oportunidades como **Analista de Infra**, **SysAdmin**, **Suporte Técnico** ou **Administração de Redes**.

---

📌 *Obrigado por visitar meu perfil! Sempre aberto para trocar ideias, aprender e colaborar.*




<!--
## Hi there 👋
-->
<!--
**terminal-junior/terminal-junior** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

Here are some ideas to get you started:

- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...
- 😄 Pronouns: ...
- ⚡ Fun fact: ...
-->
